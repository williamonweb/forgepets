...
export async function POST(request:Request){
 try{
  ...
  const ownerName=String(body.ownerName||'').trim(); const companyName=String(body.companyName||'').trim(); const email=String(body.email||'').toLowerCase().trim(); const phone=String(body.phone||'').trim(); const password=String(body.password||''); const plan: Plan=Plan.PROFISSIONAL;
  ...
  return NextResponse.json({ok:true,trialDays:2,trialEndsAt});
